...
function Monthlyreport() {
  ...
  const columns = [
    { field: "id", title: "ID", width: 50 },
    {
      ...
      title: "Doc",
      width: 50,
      customExport: (dataExport) => dataExport.toString(),
    },
    ...
    // {title: "Microchip ",field: "microchip",width: 150,renderCell: (param) =>moment.parseZone(param.value).local().format("DD/MM/YYYY"),},
    {
      ...
      width: 150,
      render: (rowData) =>
        rowData.microchip ? moment(rowData.microchip).format("DD/MM/YYYY") : "",
    },
    {
      title: "Actions",
      field: "actions",
  ...
  //--------------------------------------------------------- Get Data by date request ------------------------------------------------------------
   ...
   const onSubmit = async (formValues) => {
    try {
      ...
      setData(response.data.data); // note: renamed `pre` → `data`
    } catch (error) {
  ...
  const handleExportExcel = (evt, selectedRows) => {
    const handleMoment = (date) =>
      date
        ? moment.parseZone(date).local().format("DD/MM/YYYY hh:mm:ss A")
        : null;

    const filterData = (selectedRows?.length > 0 ? selectedRows : data).map(
      (item) => ({
        Doc: item.doc,
                ...
                <DialogContent>
                  <form>
                    <div className="row">
                      ...
                      <div class="col-12">
                        <div class="mb-3">
                          <label for="being" class="form-label">
                            Being for
                          </label>
                          <input
                            type="text"
                      ...
                      <div class="col-12">
                        <LocalizationProvider dateAdapter={AdapterDayjs}>
                          <DatePicker
                            ...
                            value={
                              update.microchip ? dayjs(update.microchip) : ""
                            }
                            onChange={(newValue) => {
